...
func (s *server) ExportVM(ctx context.Context, req *pb.ExportVMRequest) (*pb.ExportVMResponse, error) {
	artifact := fmt.Sprintf("staging/vm-%s.ova", req.VmId)
	return &pb.ExportVMResponse{ArtifactId: artifact, Message: "stubbed export from vmware"}, nil
}

func (s *server) ImportVM(ctx context.Context, req *pb.ImportVMRequest) (*pb.ImportVMResponse, error) {
	return &pb.ImportVMResponse{NewVmId: "vmware-imported-001", Message: "stubbed import to vmware"}, nil
}
